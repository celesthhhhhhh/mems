...
`;
  ...
  async function init() {
    // 1. Load settings and render UI immediately — must not depend on async ops
    try {
      loadSettings();
    } catch (err) {
      console.error('[MemoryRAG] Failed to load settings:', err);
    }

    // Render settings panel right away so it's always visible
    SettingsUI.render(settings, onSettingsChange);

    // 2. Heavy async initialization in a separate block
    try {
      extractor = new MemoryExtractor(settings);
      embedder  = new EmbeddingModule(settings);
      vectorDB  = new VectorDB(settings);
      ...
      console.error('[MemoryRAG] Async init failed:', err);
    }
  }
