...
<details class="mrag-section mrag-collapsible">
  ...
  <div class="mrag-body">
    ...
    <div class="mrag-row"><input type="range" id="mrag-ws" class="mrag-range" value="${s.rankingWeights.similarity}" min="0" max="1" step="0.05"><span class="mrag-range-val" id="mrag-ws-v">${s.rankingWeights.similarity}</span></div>
    <div class="mrag-row"><input type="range" id="mrag-wi" class="mrag-range" value="${s.rankingWeights.importance}" min="0" max="1" step="0.05"><span class="mrag-range-val" id="mrag-wi-v">${s.rankingWeights.importance}</span></div>
    <div class="mrag-row"><input type="range" id="mrag-wr" class="mrag-range" value="${s.rankingWeights.recency}" min="0" max="1" step="0.05"><span class="mrag-range-val" id="mrag-wr-v">${s.rankingWeights.recency}</span></div>
  </div>
</details>
...
`;
  ...
  async function init() {
    try {
      ...
      // Пытаемся определить путь к расширению динамически
      ...
      try {
        ...
        console.error('[MemoryRAG] Failed to load settings.html from', extensionFolderPath, e);
        // Фолбэк: если не удалось загрузить файл, создаем контейнер вручную
        if (!document.getElementById('memory-rag-settings')) {
          $("#extensions_settings").append('<div id="memory-rag-settings"></div>');
          SettingsUI.render(settings, onSettingsChange);
        }
      }
